...
class AuthAuth(models.Model):
    _name = 'auth.auth'
    _description = "Rest Authentication"
    _rec_name = "name"
    ...
    access_token_ids = fields.One2many('auth.access.token', 'auth_id', string="Access Tokens", \
                        domain=[('access_token_validity', '>', fields.Datetime.now())])
    basic_access_token_ids = fields.One2many('auth.access.token', 'basic_auth_id', string="Basic Tokens", \
                        domain=[('access_token_validity', '>', fields.Datetime.now())])
    ...
    access_type = fields.Selection([('oauth1', 'OAuth1'), ('oauth2', 'OAuth2'), ('basic', 'Basic')], default='oauth2')
    ...
    def create(self, vals_list):
        ...
        return super(AuthAuth, self).create(vals_list)
    ...
    def update_key_secret(self):
        ...
        """
        for rec in self:
            rec.consumer_key = rec.generate_token()
            rec.consumer_secret = rec.generate_token()
    ...
    def get_authorize_user(self, key):
        ...
        """
        if key:
            return self.sudo().search([('consumer_key', '=', key), ('access_type', '=', 'oauth1')], limit=1)
        return False
    ...
    def get_authorize_user_request_token(self, request_token):
        ...
        """
        if request_token:
            return self.sudo().search([('request_token_ids', '=', request_token), ('access_type', '=', 'oauth1')], limit=1)
        return False
    ...
    def key_validation(self, key):
        ...
        """
        if key:
            valid = self.sudo().search([('consumer_key', '=', key), ('access_type', '=', 'oauth1')], limit=1)
            if valid:
                return True
        return False
    ...
    def generate_oauth_signature(self, params, method, base_url):
        ...
        parse_url.path,)
        base_url = urlparse.quote(base_url).replace('+', '%20').replace('/', '%2F')
    ...
    def check_timestamp_auth(self, timestamp, nonce):
        ...
        if float(timestamp) < time.time() - valid_window or float(timestamp) > time.time() + valid_window:
            return False
        for n, t in list(self.used_nonce.items()):
            if float(t) < time.time() - valid_window or float(timestamp) > time.time() + valid_window:
                del self.used_nonce[n]
        if nonce in list(self.used_nonce):
            return False
        self.used_nonce.update({nonce: timestamp})
        return True

    def send_restapi_to_user(self):
        mail_template = self.env.ref('restapi.email_template_send_key_to_user', raise_if_not_found=False)
        ctx = {
            'user': self.env.user.email,
            'default_model': 'auth.auth',
            ...
            'model_description': "Authentication Details",
        }
...
class AuthAuthToken(models.Model):
    ...
    auth_id = fields.Many2one('auth.auth', 'Auth')
    basic_auth_id = fields.Many2one('auth.auth', 'Basic Auth')
...
class AuthRequestToken(models.Model):
    ...
    def is_valid_request(self, request_token, verifier, auth=False):
        """
            Validates, whether request token and verifier is valid.
        """

...
class RestApi(http.Controller):
    ...
    def auth(self, **kwargs):
        ...
        auth_auth = obj.search([('consumer_key', '=', kwargs.get('client_id')), ('access_type', '=', 'oauth2')])
        if not auth_auth:
            return self.get_response(401, '401', {"code": 401, "message": "Invalid Credentials."})
        redirect_uri = False
        if kwargs.get('redirect_uri'):
            redirect_uri = urllib.parse.unquote(kwargs.get('redirect_uri'))
    ...
    @http.route(['/restapi/1.0/common/basic/access_token'], \
        type="http", auth="public", csrf=False, website=True)
    def auth_basic_access(self, **kwargs):
        """
        ...
        auth_auth = obj.search([('consumer_key', '=', kwargs.get('client_id')), ('consumer_secret', '=', kwargs.get('client_secret')), ('access_type', '=', 'basic')], limit=1)
        if not auth_auth:
            return self.get_response(401, '401', {"code": 401, "message": "Invalid Credentials."})
        auth_auth.basic_access_token_ids = [(0, 0, {'access_token': access_token, 'auth_id': False, 'access_token_validity': access_token_validity})]

        result = self.get_response(200, '200', {"access_token": access_token,
    ...
    def auth_access(self, **kwargs):
        ...
        access_token_validity = datetime.now() + timedelta(minutes=30)
        token_validity = (access_token_validity - datetime.now()).total_seconds()
        auth_auth = obj.search([('consumer_key', '=', kwargs.get('client_id')), ('consumer_secret', '=', kwargs.get('client_secret')), ('access_type', '=', 'oauth2')])
        ...
        if kwargs.get('grant_type') == 'refresh_token':
            auth_auth = obj.search([('id', '=', auth_auth.id), ('refresh_token', '=', kwargs.get('refresh_token')), ('access_type', '=', 'oauth2')])
            if not auth_auth:
            ...
            try:
                ...
                if request.httprequest.headers.get('Accept') == 'application/xml' or request.httprequest.content_type == "application/xml":
                    if isinstance(data, dict):
                        ...
                        data = dicttoxml.dicttoxml(data,root=False)
                    response.data = data
                    response.mimetype = 'application/xml'
    ...
    def _get_credentials(self, kwargs):
        ...
        if request.httprequest.headers.get('Authorization'):
            ...
            elif 'Basic' in request.httprequest.headers.get('Authorization'):
                params = {'basic_access_token': request.httprequest.headers.get('Authorization').split(' ')[1]}
            else:
        ...
        if kwargs.get('oauth_token'):
            ...
            if user:
                ...
                if request_token.callback_uri:
                    return werkzeug.utils.redirect(add_params_to_uri(urllib.parse.unquote(request_token.callback_uri),{'oauth_verifier':request_token.token_verifier,'oauth_token':request_token.request_token}))
                else:
    ...
    def auth_access_token(self, **kwargs):
        ...
        elif datetime.strptime(valid_request.request_token_validity.strftime(DEFAULT_SERVER_DATETIME_FORMAT), DEFAULT_SERVER_DATETIME_FORMAT) < datetime.strptime(datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT), DEFAULT_SERVER_DATETIME_FORMAT):
    ...
    def valid_authentication(self, kwargs):
        ...
        if kwargs.get('oauth_signature'):
            ...
            access_token = request.env['auth.access.token'].sudo().search([('access_token', '=', credentials['access_token'])], limit=1)
            if not access_token or (access_token.auth_id and access_token.auth_id.access_type != 'oauth2') or datetime.strptime(access_token.access_token_validity.strftime(DEFAULT_SERVER_DATETIME_FORMAT), DEFAULT_SERVER_DATETIME_FORMAT) < datetime.strptime(datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT), DEFAULT_SERVER_DATETIME_FORMAT):
        ...
        elif credentials.get('basic_access_token'):
            access_token = request.env['auth.access.token'].sudo().search([('access_token', '=', credentials['basic_access_token'])], limit=1)
            if not access_token or (access_token.basic_auth_id and access_token.basic_auth_id.access_type != 'basic') or datetime.strptime(access_token.access_token_validity.strftime(DEFAULT_SERVER_DATETIME_FORMAT), DEFAULT_SERVER_DATETIME_FORMAT) < datetime.strptime(datetime.now().strftime(DEFAULT_SERVER_DATETIME_FORMAT), DEFAULT_SERVER_DATETIME_FORMAT):
                return False, False, self.get_response(401, str(401), {'code': 401, 'message': 'Access Token Invalid'})
            user, auth = access_token.basic_auth_id.user_id, access_token.basic_auth_id
    ...
    def generate_report(self, xml_id, ids):
        ...
        try:
            result, format = request.env['ir.actions.report'].with_user(SUPERUSER_ID)._render_qweb_pdf(xml_id, ids)
            if not result:
                tb = sys.exc_info()
                self_reports['exception'] = 'RML is not available at specified location or not enough data to print! %s' % tb
            self_reports['result'] = result
            self_reports['format'] = format
            self_reports['state'] = True
            self_reports.update({'id': ids})
        except Exception as exception:
            self_reports['exception'] = '%s' % (exception)
            self_reports['state'] = True

        exc = self_reports['exception']
        if exc:
            raise UserError('%s' % (exc))
        if self_reports['state']:
            if tools.config['reportgz']:
                ...
                if isinstance(result, str):
                    ...
                    res2 = result
            if res2:
                self_reports['result'] = binascii.b2a_base64(res2).decode('utf-8')
        return self_reports
    ...
    def call_report(self, xml_id, id=None, **kwargs):
        ...
        try:
            if not self.evaluate(kwargs.get('group')):
                for id in ids:
                    data = self.generate_report(xml_id, id)
                    datas.append(data)
            else:
                data = self.generate_report(xml_id, ids)
                datas.append(data)

        except Exception as e:
            return self.get_response(403, '403', {'error': {'code': 403, 'message': str(e)}})
        if datas and not len(datas) > 1:
            datas = datas[0]
        return self.get_response(200, str(200), {'report': datas})

    def eval_all_strings(self, data):
        if isinstance(data, dict):
            return {key: self.eval_all_strings(value) for key, value in data.items()}
        elif isinstance(data, list):
            return [self.eval_all_strings(item) for item in data]
        elif isinstance(data, tuple):
            return tuple(self.eval_all_strings(item) for item in data)
    ...
    def remove_tags(self, dic):
        for k, v in dic.items():
            if isinstance(v, dict):
                dic[k] = self.remove_tags(v)
            if "items" in dic.keys():
                ...
                try:
                    for i in dic["items"]:
                        if "item" in i.keys():
                            for temp in i["item"]:
                                if isinstance(temp, dict) and "ids" in temp.keys():
                                    try:
                                        ids = [i for i in eval(temp["ids"])]
                                    except TypeError:
                                        ids = eval(temp["ids"])
                                    i["item"][i["item"].index(temp)] = ids
                            items.append(tuple(i["item"]))
                    return items
                except AttributeError:
                    for temp in dic["items"]["item"]:
                        if isinstance(temp,dict) and "ids" in temp.keys():
                            try:
                                ids = [i for i in eval(temp["ids"])]
                            except TypeError:
                                ids = eval(temp["ids"])
                            dic["items"]["item"][dic["items"]["item"].index(temp)] = ids
                    items.append(tuple(dic["items"]["item"]))
                    return items
        return dic
    ...
    def perform_request(self, object, method=None, id=None, kwargs={}, user=None):
        ...
        if request_data and (request.httprequest.headers.get('Accept') == 'application/xml' or request.httprequest.content_type == "application/xml"):
            try:
                tree = etree.fromstring(request_data)
            except Exception as e:
                return self.get_response(400, '400', {'error': {'code': 400, 'message': str(e)}})
            if tree.tag == "vals" or tree.tag == "args" or tree.tag == "kwargs":
                requessted_payload = xmltodict.parse(request_data, attr_prefix='', cdata_key='')
                formated_dic = self.remove_tags(requessted_payload)
                formated_payload = self.eval_all_strings(formated_dic)
                request_data = str(formated_payload).encode('utf-8')
            else:
                return self.get_response(400, '400', {'error': {'code': 400, 'message': 'Bad Request'}})
        ...
        if method in ('create', 'write'):
            for f in ['vals', 'args']:
                ...
                if not payload:
                    payload = request_data and self.evaluate(request_data).get(f) or {}
                if payload:
                    break
            if not payload:
                payload = request_data and self.evaluate(request_data) or {}
            if payload:
                payload = type(payload) is list and payload[0] or payload

        if request_data and type(self.evaluate(request_data)) is dict:
            kwargs.update(self.evaluate(request_data))
        ...
        if self._METHOD_WITH_ARGS_KWARGS.get(method):
            ...
            if method == 'check_access_rights':
                args_list = []
                for arg in args['arg']:
                    evaluate_arg = self.evaluate(kwargs.get(arg))
                    if isinstance(evaluate_arg, list) and len(evaluate_arg) > 1:
                        for eval_arg in evaluate_arg:
                            args_list.append([eval_arg])
                    else:
                        args_list.append(evaluate_arg)
                arguments.extend(args_list)
            else:
            ...
            k_arguments = kwargs.get('context') and {'context': self.evaluate(kwargs['context'])} or {}
        else:
            if ids:
                arguments.append(ids)
            arguments.extend((kwargs.get('args') and self.evaluate(kwargs['args'])) or (request_data and self.evaluate(request_data).get('args')) or [])
            k_arguments = (kwargs.get('kwargs') and self.evaluate(kwargs['kwargs'])) or (request_data and self.evaluate(request_data).get('kwargs') or {})
        try:
            if method == 'check_access_rights':
                for arg in arguments:
                    data = model.execute_kw(request.cr.dbname, user.id, object, method, arg, kw=k_arguments)
            else:
            ...
            return self.get_response(403, '403', {'code': 403, 'message': "Connection Error"})
        except Exception as e:
        ...
        if request.httprequest.headers.get('Accept') == 'application/xml' or request.httprequest.content_type == "application/xml":
            ...
            return result
        datas = json.dumps(datas, default=date_utils.json_default)
        result = self.get_response(request_code, str(request_code), datas)
        if request.httprequest.headers.get('Accept') == 'application/json' or request.httprequest.content_type == "application/json":
            result.mimetype = 'application/json'
        return result
    ...
    def buscar_usuario(self, **kwargs):
        ...
        access_token_validity = datetime.now() + timedelta(minutes=30)
        #auth.basic_access_token_ids = [(0, 0, {'access_token': access_token, 'auth_id': False, 'access_token_validity': access_token_validity})]
        return self.get_response(200, '200', {
            "id": partner_id.id,
